...
app.post('/nota',function(req,res){
    let sql="SELECT NOMER_TRANSAKSI,DATE,user.NAMA,user_toko.NAMA_TOKO,ALAMAT_PEMBELI,ALAMAT_LANJUT,transaksi.BERAS,transaksi.MINYAK,transaksi.GULA,transaksi.GARAM,transaksi.TELUR,TOTAL,stok_toko.HARGA_BERAS,stok_toko.HARGA_MINYAK,stok_toko.HARGA_TELUR,stok_toko.HARGA_GULA,stok_toko.HARGA_GARAM from transaksi INNER JOIN user_toko on transaksi.ID_TOKO = user_toko.ID_TOKO INNER JOIN user on transaksi.ID_PEMBELI = user.ID INNER JOIN stok_toko on transaksi.ID_TOKO = stok_toko.ID_TOKO WHERE NOMER_TRANSAKSI = '"+req.body.id+"'"
    let query = connection.query(sql,(err,result)=>{
        console.log(JSON.stringify(
            {"status" : 200, "error" : null,"response" : result}
        ));
